...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('#data_consulta').datetimepicker({
            ...
            debug: false,
            format: 'DD/MM/YYYY',
            minDate: '<?php echo $this->bateria_atual[0]['data_inicio']; ?>',
        ...
        $('#completo').on('change', function(){
            console.log('importante: ' + $('#data_consulta').val() != '');
            console.log($('#data_consulta').val());


                //verificar essa porra ai em baixo!
            if(moment($('#data_consulta').val(), "YYYY-MM-DD HH:mm:ss")){
                var dia = moment($('#data_consulta').val(), "YYYY-MM-DD HH:mm:ss");
                var dia_semana = dia.format('dddd');
            } else {
                swal("Atenção!", "Selecione uma data para a consulta.", "error");
